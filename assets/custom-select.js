if (!customElements.get('custom-select')) {
  class CustomSelect extends HTMLElement {
    ...
    hideListbox() {
      ...
      // if (this.closest('.option-selector[data-default]') != null) {
      //   // this.options[0].click()
      //   // this.closest('variant-picker').updateAddToCartButton()
      //   const selectedOptions = Array.from(this.options).filter(option => option.getAttribute('aria-selected') === 'true');
      //   const detail = { selectedValue: selectedOptions[0].dataset.value };
      //   this.dispatchEvent(new CustomEvent('change', { bubbles: true, detail }));
      //   // this.closest('variant-picker').dispatchEvent(new CustomEvent('change', { bubbles: true, detail }));
      // }
    ...
    if (option !== this.selectedOption && !option.hasAttribute('disabled')) {
        ...
        if ( option.dataset.color != undefined ) {
          this.button.querySelector('.swatch-circle').style.setProperty('--color-bg', option.dataset.color);
        }

        if ( option.dataset.variantId != undefined ) {
          this.button.setAttribute('data-id', option.dataset.variantId)
        }

        // Update the button text and set the option as active.
        this.button.querySelector('.text-start').textContent = option.dataset.value;
        this.listbox.setAttribute('aria-activedescendant', option.id);
        this.selectedOption = document.getElementById(option.id);

        if (this.querySelector('.custom-select__btn').dataset.variant != undefined) {
          this.querySelector('.custom-select__btn').dataset.variant = option.dataset.variant;
        }
        ...
        if (this.nativeSelect) {
          ...
          const detail = { selectedValue: option };
          this.dispatchEvent(new CustomEvent('change', { bubbles: true, detail }));
        }
      }
